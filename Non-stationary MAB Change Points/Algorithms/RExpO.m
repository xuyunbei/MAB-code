...
function gainREXP3 = RExpO(environment, batchSize, gamma)
...
for t = 1:Horizon;
    [ArmToPlay, p, w] = REXP3_RecommendArm_Original(w, gamma, t, batchSize); % Arm Recommanded by REXP3
    reward = rand() < environment(t,ArmToPlay); % Play the recommanded arm
    [w, gainREXP3] = EXP3_ReceiveReward_Original(w, p, reward, NbrArms, gamma, ArmToPlay, gainREXP3); % Update the chosen arm
end
